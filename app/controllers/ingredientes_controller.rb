class IngredientesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @ingrediente.update(ingrediente_params)
        format.html { redirect_to @ingrediente, notice: 'Ingrediente atualizado com sucesso!' }
  ...
  def destroy
    @ingrediente.destroy
    respond_to do |format|
      format.html { redirect_to ingredientes_url, notice: 'Ingrediente excluido com sucesso!' }
  ...
  private
    ...
    def ingrediente_params
      params.require(:ingrediente).permit(:nome)
    end
end
